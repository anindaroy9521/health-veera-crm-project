...
def lambda_handler(event, context):
    ...
    if key.endswith("/"):
        return {"statusCode": 200, "message": "Folder event ignored"}

    # Process only JSON and CSV
    if not (key.lower().endswith(".json") or key.lower().endswith(".csv")):
        return {"statusCode": 200, "message": "Unsupported file type ignored"}
    ...
    if "/orders/" in key:
        ...
        return {"statusCode": 400, "message": f"Unsupported entity path: {key}"}
    ...
    return {"statusCode": 200, "message": "Message sent to SQS"}
